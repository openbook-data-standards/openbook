...
FIXTURE = json.loads(
    (Path(__file__).resolve().parents[3] / "examples" / "fixture.example.json").read_text(
        encoding="utf-8"
    )
)
MARKET = json.loads(
    (Path(__file__).resolve().parents[3] / "examples" / "market.example.json").read_text(
        encoding="utf-8"
    )
)
...
def test_update_local_in_sync() -> None:
    assert main(["update", "--local"]) == 0
...
def test_update_detects_stamp_drift(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    stamp = tmp_path / "openbook-spec-version"
    stamp.write_text("not-a-version\n", encoding="utf-8")
    monkeypatch.setattr(spec_mod, "STAMP_PATH", stamp)
    # update.check reads spec_version_stamp from spec_mod.STAMP_PATH via import
    ...
    problems = update_mod.check(spec_root=Path(__file__).resolve().parents[3])
