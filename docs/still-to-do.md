...
Questions recorded through **Q152**. Catalog pass closed and on the wire.
...
game is down. The number is games won (integer, 0 allowed). Count field
names wait. Toss elected still waits.
...
- Whether the count field reuses `total`.
